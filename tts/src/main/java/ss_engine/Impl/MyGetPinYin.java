...
public class MyGetPinYin implements ICharactorTool {
	...
	private static String getSpellByCode(int code) {
		...
		Integer key = spellTree.floorKey(code); // 找到最接近的拼音的key
		if (key != null) {
	...
	public List<String> getNcnFullSpell(String cnStr) throws Exception {
		...
		for (int i = 0; i < py.length; i++) {
			try {
				result.add(getFullSpell(py[i]) + ".wav");
			} catch (Exception e) {
				throw e;
			}
		}
